...
function CartItem() {
  ...
  useEffect(() => {
    Api.get(`cart/${userId}/${pageNumber}`)
      .then(response => {
        console.log(response.data.data.cartItems, 'cartList')
        ...
        {cart.map((item) => (
          <div key={item.id}>
            <div className={`cart-item sm:w-[792px] border-t border-x rounded-t-lg sm:text-[14px] sm:p-4 sm:mt-12 ${item.outOfStock ? 'border-none' : ''}`}>
              <div className="flex ">
                <img className={`sm:w-[96px] sm:ml-4 ${item.outOfStock ? 'opacity-25' : ''}`} src={item.product.image_url} alt={item.name} />
                <div className="sm:ml-4">
                  <div>
                    <h1 className={`text-justify ${item.outOfStock ? 'opacity-25' : ''}`}>{item.product.name}</h1>
                  </div>
                ...
                <div className={`sm:ml-auto ${item.outOfStock ? 'opacity-25' : ''}`}>
                  <h1 className="text-end font-semibold">AED {item.product.price * item.quantity}</h1>
                  <div className="flex sm:mt-2">
                    <div className="sm:text-[14px] sm:leading-4 font-normal text-[#A8A8A8] line-through">
                      AED {item.product.beforePrice * item.quantity}
                    </div>
                    {item.product.offer &&
