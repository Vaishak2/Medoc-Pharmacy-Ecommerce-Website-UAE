  ...
  return (
    <div>
      ...
      <div className='sm:w-[405px] sm:text-justify sm:text-14px] '>
        ...
        <h1>Phone Number</h1>
        <div
      className={`flex border sm:mt-2 rounded-lg ${
        isFocused ? 'border-[#304BA0]' : ''
      }`}
    >
      <select
        className={`sm:h-[50px] focus:outline-none pl-4 rounded-l-lg ${
          isFocused ? 'border-[#304BA0]' : ''
        }`}
        name=""
        id=""
      ...
      <input
        className={`sm:w-[326px] pl-4 py-3 rounded-r-lg focus:outline-none placeholder:text-[16px] placeholder:leading-6 placeholder:tracking-tighter ${
          isFocused ? 'border-[#304BA0]' : ''
        }`}
        type="phone"
